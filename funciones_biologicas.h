#ifndef FUNCIONESBIO_H
#define FUNCIONESBIO_H
...
//funciones del modelo de golomb
...
//funciones del modelos de hogkin-huxey
...
double hh_minf(double v,double *constantes);
double hh_hinf(double v,double *constantes);
double hh_ninf(double v,double *constantes);
double hh_tm(double v,double *constantes);
double hh_th(double v,double *constantes);
double hh_tn(double v,double *constantes);
...
double hh_minf(double v,double *constantes){
	return 1.*hh_alfam(v)/(hh_alfam(v)+hh_betam(v));
	}
	
double hh_hinf(double v,double *constantes){
	return 1.*hh_alfah(v)/(hh_alfah(v)+hh_betah(v));
	}
	
double hh_ninf(double v,double *constantes){
	return 1.*hh_alfan(v)/(hh_alfan(v)+hh_betan(v));
	}

double hh_tm(double v,double *constantes){
	return 1./(hh_alfam(v)+hh_betam(v));
	}
	
double hh_th(double v, double *constantes){
	return 1./(hh_alfah(v)+hh_betah(v));
	}
	
double hh_tn(double v,double *constantes){
	return 1./(hh_alfan(v)+hh_betan(v));
	}
...
double ml_cor_k(double *v,double *constantes){
	double x;
	x = constantes[2]*v[1]*(v[0]-constantes[3]);
	return x;
}
